...
const useFirebase = () => {
  ...
  const LogInUsingGoogle = (navigate, destination) => {
    setIsLoading(true);
  ...
  useEffect(() => {
    axios
      .get(
        `https://desolate-brushlands-67419.herokuapp.com/users/${user?.email}`
      )
      .then((res) => {
  ...
  const SaveUserDb = (name, email, method) => {
    fetch("https://desolate-brushlands-67419.herokuapp.com/users", {
      method: method,
      headers: {
